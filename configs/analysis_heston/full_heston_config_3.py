...
class FullConfig:
    ...
    # ============================================================
    # 3 低波牛市环境（Low-Vol Bull Market）
    # ------------------------------------------------------------
    # • 微笑：存在但较浅，IV 曲面平滑
    # • 波动率：冲击后迅速均值回归，长期处于低波状态
    # • 走势：路径干净，上涨趋势明确，极端尾部风险被压缩
    # • 对标：2017-2019 年 SPX、防御型股票、稳定期 ETF
    # ------------------------------------------------------------
    kappa: float = 3.0  # 回归速度快，vol shock 很快消失
    theta: float = 0.03  # 长期波动约 17%
    sigma: float = 0.20  # vol-of-vol 较低，smile 被压平
    rho: float = -0.40  # skew 存在但不尖锐
    # ============================================================
